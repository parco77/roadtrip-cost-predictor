...
This module serves JSON and nothing else. The React UI is a separate deployment on a static
host, which is why CORS is configured below and why there is no StaticFiles mount.

Everything it loads at import time - both model bundles and both CSVs - sits beside it under
backend/, so this folder is the whole of what gets deployed. The scripts that produce those
four files live in ml/ and write straight into it:

    python ../ml/scripts/build_cities.py            # cities with real lat/lon    -> data/
    python ../ml/scripts/build_fuel_prices.py       # per-state fuel prices       -> data/
    python ../ml/scripts/build_wide_dataset.py      # 25,000 trips
    python ../ml/scripts/fetch_real_distances.py    # 840 REAL road distances from OSRM
    python ../ml/scripts/build_distance_matrix.py   # -> frontend/public/city_distances.json
    python ../ml/scripts/train_pipeline.py          # -> models/pipeline.joblib (the sub-models)
    python ../ml/scripts/train_models.py            # -> models/model.joblib    (band + curve)
...
Endpoints
    ...
    GET  /                       what this service is, for anyone who opens the bare URL
"""
...
# The UI is served from a different origin, so the browser preflights every call.
# ALLOWED_ORIGINS is a comma-separated list supplied by the host; it defaults to "*" so that a
# fresh clone, a local build and a preview deploy all work with no configuration. Nothing here
# is authenticated and no response varies by caller - there is no session for a wildcard to
# expose - but naming the real origin in production is still worth the one environment variable.
...
@app.get("/")
def index():
    """Opening the service URL in a browser should explain what it is, not 404."""
    return {
        "service": app.title,
        "version": app.version,
        "docs": "/docs",
        "health": "/api/health",
        "ui": os.getenv("FRONTEND_URL") or "deployed separately",
    }
