"""
Emit the Week 9 / Week 10 HTML sections of the full project report, straight from the saved JSON.
...
#                                   PART 11 - WEEK 9
# ==========================================================================================
w('<h1 class="part"><span class="num">Part 11 &middot; Week 9</span>Task 5 &mdash; evaluation, '
  'validation and tuning</h1>')

w("<p>Weeks 1&ndash;8 built models. Week 9 measures them properly, applying the same six steps to "
...
w("<h2>11.1 How everything was measured</h2>")
...
w("<h2>11.2 Step 1 &mdash; the metrics, and what each is for</h2>")
...
w("<p>Precision, recall and F1 are <strong>macro-averaged</strong> (every class weighted equally) "
  ...
  "all. Part 11.10 shows exactly that failure on the original dataset.</p>")

# ---------------------------------------------------------------- regression table
w("<h2>11.3 Steps 1 and 4 &mdash; the regression comparison</h2>")
w("<p>Target <span class='mono'>total_trip_cost</span>. Feature set: the model this project "
  "already ships (Week 6). Every row sees exactly those columns and that split, so the table "
  "compares <strong>models</strong>, not feature sets. The first row is the Week 6 ablation "
...
w(f"<p>The largest single effect in the table is not a model at all. Removing one engineered "
  ...
  f"than between the best and worst <em>model</em>. That is Week 6's argument restated: an "
  f"additive model cannot express a product, and no amount of model complexity substitutes for "
  f"handing it over.</p>")

# ---------------------------------------------------------------- overfit table
w("<h2>11.4 Step 2 &mdash; overfitting and underfitting</h2>")
...
w("<p>So <strong>on this target the train/test comparison is close to uninformative</strong>. It "
  ...
  "the <span class='mono'>cost_band</span> classifier below, and the toll sub-model in Part 12, "
  "where the same trees show gaps of 0.16 to 0.37 and are correctly flagged.</p>")
w("</div>")

# ---------------------------------------------------------------- CV table
w("<h2>11.5 Step 3 &mdash; 5-fold cross-validation, and why the spread is the point</h2>")
...
w("<p>Every spread is tiny &mdash; CV standard deviations of 1e&minus;5 to 4e&minus;4. With 20,000 "
  ...
  "the models.</strong> Part 11.8 repeats it on 1,140 rows, where it does.</p>")
...
# ---------------------------------------------------------------- selection rule
w("<h2>11.6 Step 4 &mdash; choosing between models that are statistically tied</h2>")
...
tbl(["Filter", "Rule", "Why"], [
    ...
    ["3. Simplicity", "of the survivors, take the simplest",
     "Not only Occam's razor &mdash; a linear model keeps the Week 3 gradient-descent-from-scratch "
...
# ---------------------------------------------------------------- classification
w("<h2>11.7 Steps 1&ndash;4 for classification</h2>")
...
# ---------------------------------------------------------------- tuning
w("<h2>11.8 Step 5 &mdash; hyperparameter tuning</h2>")
w("<p><strong>Why not grid-search the winning model?</strong> The best regressor is "
  ...
  "&alpha; is a real search whose answer means something: <strong>Week 8 rejected Ridge by "
...
w("<p><strong>This reproduces Week 8's hand-argument mechanically.</strong> A search whose answer "
...
# ---------------------------------------------------------------- advanced models
w("<h2>11.9 Step 6 &mdash; the advanced models, in one place</h2>")
...
if orig:
    w("<h2>11.10 The same checklist on the original 1,140-row dataset</h2>")
...
w("<p><strong>And the finding the checklist did not ask for.</strong> The regression above scores "
  "R&sup2; 0.9997, but it is handed the true <span class='mono'>toll_cost</span>, "
  "<span class='mono'>parking_cost</span> and <span class='mono'>fuel_consumption_litres</span> as "
  "input features &mdash; three of the four terms of the cost formula. A user supplies none of "
  "them. Part 12 takes them away.</p>")
